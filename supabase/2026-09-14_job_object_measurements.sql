-- Explicitly persist object measurements in the atomic creation path.
begin;
alter table public.jobs add column if not exists object_kind text check(object_kind in ('apartment','house','land','commercial','other'));
alter table public.jobs add column if not exists object_details jsonb not null default '{}'::jsonb check(jsonb_typeof(object_details)='object');
...
begin
  ...
  insert into public.jobs (
    type, scheduled_date, scheduled_time, address, floor, area, object_kind, object_details, source, pest,
    ...
    v_input.area, v_input.object_kind, coalesce(v_input.object_details, '{}'::jsonb), v_input.source, v_input.pest,
