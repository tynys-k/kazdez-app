...
function Dashboard({ session, profile }) {
  ...
  async function convertLeadToJob(lead) {
    ...
    const { error } = await supabase.rpc(rpcName, {
      p_lead_id: lead.id,
      p_guarantee_months: defaultGuarantee,
    });
    if (error) {
      const message = atomicReceiptRpcUnavailable(error, rpcName)
        ? `Безопасное создание заявки из лида ещё не включено. Выполни supabase/${ATOMIC_LEAD_CONVERSION_MIGRATION} — заявка не была создана.`
        : error.message;
      showToast("Ошибка: " + message); return;
    }
  ...
  async function transferEquipment(h, newTechId, note) {
    const rpcName = "transfer_equipment_atomic";
    const { error } = await supabase.rpc(rpcName, {
      p_handout_id: h.id,
    ...
    if (error) {
      showToast(atomicReceiptRpcUnavailable(error, rpcName)
        ? `Сначала примени миграцию ${ATOMIC_EQUIPMENT_TRANSFERS_MIGRATION} в Supabase`
        : "Ошибка: " + error.message);
      return;
    }
