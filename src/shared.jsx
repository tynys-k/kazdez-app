...
const EMPLOYEE_JOB_TITLES = [
  ...
  "Бухгалтер",
  "Главный бухгалтер",
  "Специалист по государственным закупкам / тендерщик",
  "Администратор",
  "Инструктор",
  "Координатор",
  "Маркетолог",
...
const DISCOUNT_REASONS = {
  debt: "Клиент не доплатил — обещал позже",
...
const CHANGE_FIELDS = {
  ...
  role: "Роль доступа", job_title: "Должность", hired_on: "Дата приёма", is_active: "Доступ", salary_monthly: "Оклад",
...
// Типы визитов внутри заказа.
//
// Заказ — то, за что клиент заплатил. Визит — выезд. Разница важна там, где
// выездов больше одного: гарантийный и контрольный визиты денег не приносят,
// и считать их отдельными продажами — значит занижать средний чек.
const VISIT_KINDS = {
  primary: { label: "Первый выезд", short: "1-й", paid: true },
  control: { label: "Контрольный выезд", short: "контроль", paid: false },
  guarantee: { label: "Выезд по гарантии", short: "гарантия", paid: false },
  contract: { label: "Плановый по договору", short: "план", paid: true },
};

// Точки контроля на объекте.
const CONTROL_POINT_KINDS = {
  bait_station: "Приманочная станция",
  snap_trap: "Механическая ловушка",
  live_trap: "Живоловка",
  glue_board: "Клеевой монитор",
  insect_monitor: "Феромонная ловушка",
  uv_lamp: "Инсектицидная лампа",
};

// Результат осмотра. Порядок важен: он же задаёт, что тревожнее.
const CHECK_RESULTS = {
  clean: { label: "Чисто", alarm: false },
  consumed: { label: "Приманка съедена", alarm: true },
  activity: { label: "Есть активность", alarm: true },
  damaged: { label: "Станция повреждена", alarm: true },
  missing: { label: "Станция отсутствует", alarm: true },
  replaced: { label: "Заменена", alarm: false },
};
...
const ADMIN_TAB_ORDER = ["today", "jobs", "schedule", "routes", "tasks", "leads", "proposals", "clients", "contracts", "retention", "subscriptions", "repeats", "finance", "analytics", "growth", "opex", "cash", "done", "canceled", "team", "payroll", "partners", "stock", "tenders", "docs", "materials", "knowledge", "journal", "trash"];
...
const ROLE_DEFAULT_PERMISSIONS = {
  ...
  manager: ["tab.today", "tab.jobs", "tab.schedule", "tab.done", "tab.canceled", "tab.routes", "tab.tasks", "action.tasks_manage", "action.jobs_edit", "tab.leads", "tab.clients", "action.leads_edit", "tab.proposals", "tab.repeats", "tab.retention", "tab.subscriptions", "tab.partners", "action.partners_edit", "tab.materials", "tab.knowledge"],
  marketer: ["tab.today", "tab.leads", "tab.clients", "action.leads_edit", "tab.proposals", "tab.retention", "tab.analytics", "tab.growth", "tab.materials", "tab.knowledge", "tab.tasks"],
  ...
  tender: ["tab.today", "tab.tenders", "action.tenders_edit", "tab.proposals", "tab.docs", "action.docs_edit", "tab.tasks", "action.tasks_manage", "tab.materials", "tab.knowledge"],
  curator: ["tab.today", "tab.jobs", "tab.schedule", "tab.done", "tab.canceled", "tab.routes", "tab.tasks", "action.tasks_manage", "action.jobs_edit", "tab.clients", "tab.retention", "action.leads_edit", "tab.team", "tab.stock", "data.stock_self", "data.equipment", "tab.materials", "tab.knowledge"],
};
