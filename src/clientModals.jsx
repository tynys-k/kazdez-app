...
import { ClientStatusBadges } from "./workflows/ClientStatus";
import { CLIENT_LABELS } from "./workflows/clientStatusModel";
import QualityHistory, { qualityHistoryForPhone } from "./workflows/QualityHistory";
...
export function ClientProfileModal({ client, contacts = [], addresses = [], onClose, onSave }) {
  const [profile, setProfile] = useState({
    client_labels: client?.client_labels || [],
  ...
  return <Shell title={client ? "Данные клиента" : "Новый клиент"} onClose={onClose} footer={<><button className="kd-btn ghost" onClick={onClose}>Отмена</button><button className="kd-btn primary" disabled={saving || profile.phone.replace(/\D/g, "").length < 10} onClick={save}>{saving ? "Сохраняем…" : "Сохранить"}</button></>}>
    ...
    <fieldset className="wf-section"><legend>Статусы клиента</legend><div className="wf-members">{Object.entries(CLIENT_LABELS).map(([id, label]) => <label key={id}><input type="checkbox" checked={profile.client_labels.includes(id)} onChange={(e) => set("client_labels", e.target.checked ? [...profile.client_labels, id] : profile.client_labels.filter((key) => key !== id))} />{label}</label>)}</div><p className="kd-muted">«Постоянный» также назначается автоматически по числу завершённых первичных заявок.</p></fieldset>
    <div className="kd-client-form-section"><div className="kd-client-form-head"><div><div className="kd-section">Контактные лица</div><small>Сотрудники, родственники и другие контакты</small></div><button className="kd-btn ghost sm" onClick={() => setPeople((rows) => [...rows, blankContact()])}><Plus size={14} />Контакт</button></div>
      {people.length === 0 && <div className="kd-muted">Дополнительных контактов нет.</div>}{people.map((row, index) => <div className="kd-client-edit-row" key={row.id || index}><div className="kd-grid2"><Field label="Имя"><input value={row.name || ""} onChange={(e) => updateRow(setPeople, index, "name", e.target.value)} /></Field><Field label="Роль / должность"><input value={row.role || ""} onChange={(e) => updateRow(setPeople, index, "role", e.target.value)} placeholder="Управляющий, бухгалтер…" /></Field><Field label="Телефон"><input value={row.phone || ""} onChange={(e) => updateRow(setPeople, index, "phone", e.target.value)} /></Field><Field label="E-mail"><input value={row.email || ""} onChange={(e) => updateRow(setPeople, index, "email", e.target.value)} /></Field><Field label="График / примечание"><input value={row.note || ""} onChange={(e) => updateRow(setPeople, index, "note", e.target.value)} placeholder="Смена, дни работы, кого встречать" /></Field></div><button className="kd-iconbtn danger" onClick={() => setPeople((rows) => rows.filter((_, i) => i !== index))}><Trash2 size={14} /></button></div>)}
    </div>
...
export function ClientDetailsModal({ regularClientThreshold = 3, onBlock, onOpenLegalContract, qualityChecks = [], people = [], legalContracts = [], client, jobs = [], contacts = [], addresses = [], contracts = [], followups = [], events = [], leads = [], leadActivities = [], attachments = [], jobProofs = [], canEdit, onClose, onEdit, onAddNote, onUpload, onOpenAttachment, onOpenJob, onOpenContract, onOpenProof }) {
  ...
  const linkedContracts = legalContracts.filter((c) => c.client_id === client.id);
  ...
  const timeline = [
    ...
    ...linkedContracts.map((row) => ({ id: `contract-${row.id}`, at: row.created_at, type: "contract", title: `Договор № ${row.number}`, details: row.organization })),
  ...
  return <Shell title={`Клиент 360° · ${client.name || client.phone}`} onClose={onClose} footer={<button className="kd-btn primary" onClick={onClose}>Закрыть</button>}>
    ...
    <ClientStatusBadges client={client} jobs={jobs} threshold={regularClientThreshold} />
    {canEdit && onBlock && <button className="kd-btn ghost sm" onClick={() => onBlock(client)}>{client.blocked ? "Изменить блокировку клиента" : "Чёрный список"}</button>}
    <div className="kd-client360-kpis"><div><span>Заявки</span><strong>{summary.jobs}</strong><small>{summary.done} выполнено</small></div><div><span>LTV клиента</span><strong>{fmt(summary.revenue)} ₸</strong><small>за всё время</small></div><div><span>Договоры</span><strong>{linkedContracts.filter((c) => c.status === "active").length}</strong><small>активных</small></div><div><span>Контакты</span><strong>{linkedContacts.length + 1}</strong><small>{summary.openFollowups} требуют действия</small></div></div>
    <div className="kd-client360-tabs">{[{ id: "overview", label: "Обзор" }, { id: "quality", label: "Контроль и отзывы" }, { id: "timeline", label: `Хронология · ${timeline.length}` }, { id: "jobs", label: `Заявки · ${rows.length}` }, { id: "files", label: `Файлы · ${linkedAttachments.length + proofRows.length}` }].map((item) => <button key={item.id} className={view === item.id ? "on" : ""} onClick={() => setView(item.id)}>{item.label}</button>)}</div>
    {view === "overview" && <div className="kd-client-profile-grid"><section className="kd-client360-panel"><div className="kd-section"><UserRound size={16} />Контакты</div><a href={`tel:+${digits}`}>{client.phone} · основной</a>{client.email && <a href={`mailto:${client.email}`}>{client.email}</a>}{linkedContacts.map((row) => <div className="kd-client-contact" key={row.id}><strong>{row.name || row.phone || row.email}</strong><span>{[row.role, row.phone, row.email, row.note].filter(Boolean).join(" · ")}</span></div>)}</section><section className="kd-client360-panel"><div className="kd-section"><MapPin size={16} />Адреса и объекты</div>{linkedAddresses.length ? linkedAddresses.map((row, index) => <div className="kd-client-contact" key={row.id || index}><strong>{row.label || (row.fromHistory ? "Из истории заявок" : "Адрес")}</strong><span>{row.address}</span></div>) : <span>Адресов пока нет.</span>}</section><section className="kd-client360-panel"><div className="kd-section"><Building2 size={16} />Договоры</div>{linkedContracts.length ? linkedContracts.map((row) => <button className="kd-client-linkrow" key={row.id} onClick={() => onOpenLegalContract(row)}><span><strong>№ {row.number} · {row.organization}</strong><small>от {isoToRu(row.signed_on)} · {row.status === "closed" ? "Закрыт" : row.status === "draft" ? "Черновик" : "Действует"}</small></span><FolderOpen size={15} /></button>) : <span>Договоров нет.</span>}</section><section className="kd-client360-note"><div className="kd-section">Внутренняя заметка</div>{client.note && <p>{client.note}</p>}{canEdit && <><textarea rows={3} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Итог звонка, договорённость, важный контекст…"/><button className="kd-btn primary sm" disabled={!note.trim() || savingNote} onClick={saveNote}>{savingNote ? "Сохраняем…" : "Добавить в хронологию"}</button></>}</section></div>}
    {view === "quality" && <QualityHistory checks={qualityHistoryForPhone(client.phone, jobs, qualityChecks)} jobs={jobs} people={people} />}
